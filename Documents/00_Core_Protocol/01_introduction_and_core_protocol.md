...
*   **定义**: 取代原 "Logs" 概念。这是 ACP 协议规定的**强制性机制**，是实现过程透明性和协作有效性的核心。它要求 `Arena` (及其 `Cognitor`) 基于其**元认知能力**，将认知过程中的关键步骤、内部状态变化（模拟）、决策依据、对 `Uncertainty` 的处理、解释、警告和错误等，以结构化的 **`Forma`** 记录下来，形成**认知轨迹 (Cognitive Trace)**。
