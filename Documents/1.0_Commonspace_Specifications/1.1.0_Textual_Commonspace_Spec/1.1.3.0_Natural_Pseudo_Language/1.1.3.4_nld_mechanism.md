...
3. 在 NL 描述中**精确引用** NPL 上下文中的 `Datum` 句柄，解决指代模糊问题。
...
自然语言定义 (NLD) 机制，通过在 NL 主体中**同时支持 `{{datum_handle}}` 精确引用和 `{% npl NPL_Code %}` 逻辑嵌入**，显著增强了 NPL 融合自然语言的能力。这种结合利用了 NPL 的精确性来约束 NL 的灵活性，解决了指代模糊和逻辑嵌入不便的核心问题。它通过结构化的方式、增强的运行时处理机制和强制的详尽认知轨迹要求来管理交互的复杂性，有望成为构建更强大、更易用、更“有机”的 NPL 认知指令和库的关键推动力。