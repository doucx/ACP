...
*   **纯文本交互**: ACP Canvas 的 Arena Context 是纯文本。所有指令、数据和元信息都以文本形式交换。这意味着即使 `Cognitor` 通过实现 `Arena` 模拟了函数、变量等高级概念，其状态和行为也**必须**通过文本日志进行记录和维护，以确保跨载体的一致性和可审计性。
...
### Arena (运行时环境)
*   **定义**: 在 Canvas 中，`Arena` 的模拟过程主要通过当前负责执行的 `Cognitor`（通常是语言模型）**生成文本并管理 `Cell` 流**来体现。其运作方式包含以下关键方面：
    ...
    3.  **行为模拟:** `Arena` 的其他行为，如自动将自然语言路由给 Fhrsk，也是 `Cognitor` 根据对文本流和 ACP Canvas 规则的**理解和模拟**来执行的。
