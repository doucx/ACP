...
*   **`__uncertainty__`**:
	...
	*   字符串前缀： `u"苹果"` 相当于 "苹果".to_uncertainty()
...
*   **`__forma__()`**:
    *   作用: 类似`__str__`，返回该 `Uncertainty` 的一个简洁摘要描述 (`Forma`)，反映当前对其状态的理解。
...
在 NPL 中，使用日志机制来实现 `Cognitive Trace System`。

ACP 协议将记录认知过程的机制称为 **`Cognitive Trace System`**，其产物为 **`Cognitive Trace`** 。这是为了强调其作为认知活动结果 `Forma` 的本质。

在 NPL 中，生成 `Cognitive Trace` 的标准化工具对象被命名为 `Log(日志)` (例如 `Log.npl(...)`)。此命名是为了保持 NPL 语法的简洁性与借用现有编程语言日志的意象。请在阅读和使用时理解：**NPL `Log` 对象是用于创建和控制 `Cognitive Trace` 的接口。**

所有被创建的日志都是 Arena 的一部分。
...
用于在 NPL 代码中显式输出信息，而不干扰 stdout 。

#### TRACE

指示当前正在执行的是哪个认知指令（如自然语言和NPL），通过原认知指令的创建者加简略形式来引用。

需自动生成。

#### ARENA

有关 Arena 环境变化的记录。例如变量创建的声明，Cognitor 切换等。
...
`Auto` 类是 NPL 中**直接调用和引导 `Cognitor` 核心认知能力**（特别是推理和生成能力）的主要接口。它的核心机制在于将用户提供的**认知指令**（无论是自然语言描述 (`Forma`) 还是结构化条件 (`Forma`)）**视为施加于某个潜在 `Uncertainty` 上的复杂约束 (`Constraint`) 或处理目标**。`Auto` 方法会结合当前的 `Arena` (作为隐式 `Forma` 约束)，然后**请求 `Cognitor` 调用其能力**，尝试应用这些约束来管理目标 `Uncertainty`，并将其转化为一个相对确定的 `Forma` 或更明确的 `Uncertainty` 状态（例如，生成一段 NPL 代码 `Forma`、填充对象属性 `Forma`、执行一个推断出的操作）。
