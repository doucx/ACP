...
属性：
*   **`cognitor: Cognitor`**:
    * 作用：始终指向当前正在操作 `Arena` 的 `Cognitor` 的 `Forma` 句柄。

*   **`context: str`**:
    * 作用：始终指向当前 `Arena` 的内容的 `Forma` 句柄。
...
为了避免引用大量内容造成界面混乱，只需要包含：
1. 该认知指令的创建者
2. 该认知指令的核心形式
或者协商后选择其它可以唯一指向该认知指令的方式。

#### ARENA

不可直接在NPL脚本中使用。由`Cognitor`手动记录。

有关 Arena 行为声明的（如获取，释放，路由等）的认知轨迹。
