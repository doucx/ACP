...
为了满足 Cognitive Trace 的要求，与常见日志系统不同的是，在 NPL 以及 Textual Arena 中，不存在日志级别的概念，不提供日志级别的控制方法，只有日志类型的概念，相当于强制 “Verbose” 模式。
