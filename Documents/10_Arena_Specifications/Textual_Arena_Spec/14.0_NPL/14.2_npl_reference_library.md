...
由于实际的 `Forma` 直接基于 `Arena` 存在，它不需要使用 `print` 等方法输出。对它的创建本身就是在 `Arena` 中显示，也就是 **创建即输出**。见 [[12_protocol_requirements]]。

NPL 中的 `Forma` 类用于在 NPL 中，**标记**一个 Forma。

可使用 `Forma` 或 `F` 来标记一个 `Forma` 应当被作为 `Forma` 来识别。
...
-    `add_constraint(CD: Forma|Uncertainty)`: 
	- 为该 `Uncertainty` 添加一个约束。处理顺序：
		1. `CD` 如果是 `Uncertainty`，则不变，否则将被转化为 `CD: Uncertainty`。
		2. 如果 `CD: Uncertainty` 可以被 `Cognitor` 理解，则跳至 `4`。
		3. CD.add_constraint(auto 额外信息)
		4. `Cognitor` 基于其语言理解能力来解读这个 `CD: Uncertainty` 约束并应用它来缩小 `Uncertainty`。
...
- **`cognitor: Cognitor`**
	- **类型**：占位符（Placeholder）
	- **作用**：
	  - 在 `TRACE CT` 中被调用时，`cognitor` 指向的是 **当前访问 `Arena.cognitor` 的 `Cognitor`**。
	    - 具体来说，每当某个 `Cognitor` 使用或访问 `TRACE CT` 时，`Arena.cognitor` 会自动更新为该 `Cognitor` 的身份。
	  - 这种机制确保了 `Arena.cognitor` 始终反映最新的调用者信息，而不是固定的创建者信息。用来标识“当前是谁在操作”。

- **`context: str`**
	- **类型**：占位符（Placeholder）
	- **作用**：
	  - 在 `TRACE CT` 中被调用时，`context` 指向的是 **当前访问**  `Arena.context` 时的 `ArenaContext`。
...
在 NPL 以及 Textual Arena 中，不存在“级别”的概念，只有“类型”的概念。
...
在NPL脚本中使用。指示`Cognitor`记录信息。

用于在 NPL 代码中显式输出需要的信息，而不干扰 stdout 。

#### TRACE

不可直接在NPL脚本中使用。由`Cognitor`手动记录。

用于指示当前正在执行的是哪个认知指令（如自然语言和NPL）。

为了避免引用大量内容造成界面混乱，在 TRACE CT 中，只需要包含：
1. 该认知指令的创建者
2. 该认知指令的核心形式
...
可使用 `with ct.think: code` 声明此处需要记录思考内容。

有关 Cognitor 的思考内容的记录。

#### SAY

不可直接在NPL脚本中使用。由`Cognitor`手动记录。

可使用 `with ct.say: code` 声明此处需要对外表达。

强调交互性，记录 `Cognitor` 对外表达的自然语言内容。

#### ARENA

不可直接在NPL脚本中使用。由`Cognitor`手动记录。

有关 Arena 操作者的（如获取，释放，路由等）的认知轨迹。
