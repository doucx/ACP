...
* **NPL 指导深度探索**: 可以设计 NPL 指导模式（可能利用 [[15.4_builtin_libs]] 中重新定义的指导库，或特定的结构化自然语言请求）来**请求** `Cognitor` 对某个特定的**外显再现体** (`Datum`) 进行深入的意义探索。例如，指导可以要求 `Cognitor` "针对 `Datum X`，考虑其在不同上下文下的潜在 `Referent`，并列出至少三种不同的 `Interpretant` 及其推理依据"。
