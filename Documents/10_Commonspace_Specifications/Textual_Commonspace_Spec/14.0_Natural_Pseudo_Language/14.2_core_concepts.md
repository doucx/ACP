...
`Representamen` 直接基于 `Space` 存在，它不需要使用 `print` 等方法输出来显示其内容。对它的创建本身就已经在 `Space` 中显示了。见 [[11_protocol_requirements]]。
...
在 NPL 中，指向一个 `Referent` 的 `Representamen` 句柄。见 [[11_protocol_requirements]]
