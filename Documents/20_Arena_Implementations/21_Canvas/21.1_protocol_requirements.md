...
*   **类XML格式**:  Commonspace 是 类似XML格式的文本。所有指令、数据和元信息都以XML格式交换。由于Canvas是基于Cognitor运行的，因此Canvas中的XML格式相比于一般的XML格式更加松。
...
### Commonspace
*   **定义**: 在 Canvas 环境下，`Commonspace` 是 `<Canvas>` 标记中的所有 `Representamen` 内容。`<Canvas>` 标记外的内容将被排除在 `Commonspace` 外。
