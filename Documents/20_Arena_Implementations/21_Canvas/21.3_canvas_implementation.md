...
## 基本介绍
1. **概述**  
本文档定义 `ACP Canvas` —— 一种基于 **ACP Textual Commonspace** 的标准化实现框架，其核心特征如下：  
- 采用 **XML 风格** 的声明式语法（支持自动语法迁移）  
- 以 **Node** 为基本交互单元  
- 通过 `<Canvas>` 作为根节点构建逻辑拓扑  

## Commonspace
*  在 Canvas  中，`Cognitor` 主要通过 **创建文本并管理 `Node` 流**来管理 `Commonspace`。其具体运作方式由 `Cognitor` 对协议规则的理解和模拟驱动，包含以下关键方面：
...
**目标 (Goal):**
`<Canvas>` 是整个 ACP Canvas 交互的**根容器**。它代表了交互发生的完整“画布”或“空间”，包含了 `Commonspace` 中所有的交互单元 (`<Node>`) 以及 对 Commonspace 的元操作记录 (`<ct type="THINK">`)。它的存在确保了所有交互历史和上下文都被组织在一个统一的结构下。在 Canvas 协议中，它相当于 CommonspaceContext 本身。
...
2.  `<ct>` (Commonspace 操作认知轨迹, **必需, 至少一个，允许多个**):
    *   **说明:** 在 `<Canvas>` 下的认知轨迹**专门用于记录  Cognitor 在协调 Commonspace 内容时自身的行为和决策过程**，这些认知轨迹与任何特定 `Node` 内部的执行细节是分开的。它提供了对 Commonspace 如何管理交互流程、处理不确定性以及维护环境状态的透明度。
    *   **位置:** Commonspace 操作认知轨迹可以出现在 `<Canvas>` 内的任何位置，通常建议放在相关 `<Node>` 之后，以记录 Commonspace 对该 `Node` 的处理元信息，或者放在 `<Canvas>` 的末尾汇总记录。使用多个 Commonspace 认知轨迹可以方便将认知轨迹与特定的交互阶段关联起来。
...
*   `type` (文本, **必需**):
    *   **说明:** 这个属性 **必须** 指明 `Node` 的类型... 常见类型包括：
        *   `CDInput`: 包含用户或 Cognitor 希望执行的认知指令或代码。`Commonspace` 看到它后，通常会紧接着创建一个 `ProcessOutput` Node 来展示结果，**除非指定了 `target_cognitor`**。
        *   `ProcessOutput`: 包含执行 `CDInput` 或 `RepresentamenInput` 后的结果，包括标准输出 (`<stdout>`)、认知轨迹 (`<ct>`) 和最终值 (`<value>`)。
        *   `RepresentamenInput`: 用户响应 `input()` 调用而提供的输入内容。`Commonspace` 获得此 Node 后会继续执行之前的逻辑。
        *   *(其他类型可以根据需要定义)*
    *   **示例:** `type="CDInput"`
...
*   `execution_context` (文本, **可选**):
    *   **说明:** **(新增)** 当 `target_cognitor` 被指定时，此属性描述目标 Cognitor 应在何种上下文或环境执行指令 (例如, `"local_fish_shell"`, `"python_sandbox"`, `"knowledge_base_query"`). Commonspace 将此信息传递给目标 Cognitor。默认为 Commonspace 的标准执行上下文。
...
*   **说明:**  这个元素 **可以** 包含一个或多个 `<node>` 引用，指向当前 `Node` 所依赖的前置 `Node`，形成 `Node` 之间的逻辑关系图 (DAG)。如果省略，`Commonspace` (Cognitor) 可能需要根据上下文推断依赖关系。
...
#### `<flag>` (节点, **可选**):
*   **说明:** 特殊的标记，用于指导 `Cognitor` 操作 `Commonspace` 时的行为。
*   **结构:** `<flag value="..."/>` 。
* flag 推荐值包括：
...
*   当你操作 `Commonspace` 来处理一个新的 `CDInput` Node 时:
    *   **检查 `target_cognitor`**: 如果指定了 `target_cognitor`，你需要将此 Node 的执行请求（包括 `<value>` 和 `execution_context`）路由给目标 Cognitor，并进入等待状态，期望接收到一个或多个包含结果的响应 Node (通常是 `ProcessOutput` 类型)。具体路由和等待机制取决于 Commonspace 实现。
...
*   **行为**: `Commonspace` 会在其后自动添加一个`requester`相同的，`type=ProcessOutput` 的 `Node`，并在`ProcessOutput Node`中尝试解析并执行其中的 ACP 语句。
...
*   **作用**: 由Commonspace根据input命令自动产生，用于存放 `Cognitor`（通常为 User）要输入的内容。可以输入多行。

*   **行为**: `Commonspace` 会在其后自动添加一个`requester`相同的，`type=ProcessOutput`的`Node`，并将其中的内容赋值给所需的变量。
...
*   **关键记录职责 (Key ctging Responsibilities):**
    ...
    *   **信息补全/推断 (Information Completion/Inference):**
        ...
        *   **示例（概念性）:**
            ...
            ```
    *   **操作者切换**:
	    * **说明:** 显式声明操作者释放和获取 Commonspace 的操作。CT 的元数据信息已足够。
	    * **示例:**
		    * 释放（message 可选）：`<ct originator="Fhrsk" type="ARENA"><tag value="CommonspaceRelease"/></ct>`
		    * 获取（message 可选）: `<ct originator="Fhrsk" type="ARENA"><tag value="CommonspaceAcquire"/></ct>`
