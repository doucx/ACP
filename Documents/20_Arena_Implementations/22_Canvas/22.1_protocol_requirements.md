...
*   **核心协议要求**:
    ...
    2.  **Node 处理**: **必须**定义如何处理不同类型的 `<Node>` (`CDExec`, `ProcessOutput`, `FormaInput` 等)，包括何时创建新的 Node 以及 Node 之间的状态转换逻辑（例如，`CDExec` 后通常跟随 `ProcessOutput`，`input()` 调用需等待 `FormaInput`）。
    3.  **指令路由**: 对于 `CDExec` Node，Cognitor **必须**有明确的机制来决定是由其内部模拟执行、路由给特定 PersonaCognitor（如 Fhrsk）、还是根据 `target_cognitor` 属性委托给其他指定 Cognitor 执行。
