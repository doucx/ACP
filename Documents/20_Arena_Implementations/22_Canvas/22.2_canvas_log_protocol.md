# ACP Canvas 日志条目格式
本规范定义了 ACP Canvas 中 **认知轨迹 (`Cognitive Trace`)** 条目的推荐结构，旨在同时适应人类 Cognitor 的输入习惯和当前主流 LLM Cognitor 的实际输出特性（如倾向于自然语言叙述、难以提供精确内部状态、可能存在幻觉等），同时保持足够结构化以供分析。这些条目是核心协议中 `Cognitive Trace System` 在 Canvas 环境下的具体体现。

注：为了便于理解，认知轨迹 暂时和 日志 等价 (0.4.0 beta 5)。
...
7. **`flag`** (String, 可选, 可多个)
	* 提供日志对该类别信息的补充说明。推荐值包括：
        *  `CreateNeed`: 标记需要创建一个`Node`。可能会影响`Node`的`flags`。
