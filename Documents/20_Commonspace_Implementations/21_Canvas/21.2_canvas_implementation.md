...
* **核心子节点**:
    ...
    * `<plan_steps>` (**必需**): 包含一系列步骤描述，每个步骤可以是：
        * 自然语言描述的任务。
    * `<ct>` (推荐): 记录规划过程的思考和决策。
* **行为**: `Cognitor` 创建或接收此节点，理解规划内容，并按照步骤顺序创建后续的 `DirectiveNode` 节点用于详细化每一步的内容，基于此创建的`DirectiveNode` 节点必须依赖当前 `PlanNode`。
* **示例**:
    ```xml
...
* **目标**: 封装一个具体的认知指令（NPL 或自然语言），要求 `Cognitor` 进行处理，该 `Cognitor` 可以是自己。
...
* `<link />`:
    * `target` (文本, **必需**): 引用目标节点的 `node_id`。`node_id` 是 `{origin}-{seq}` 组成的结构，用于唯一指向一个 `Node` 。
