...
本文档定义了 NPL (Natural Pseudo Language) 在 **ACP Canvas** 环境下的扩展功能和交互模式。这里的 NPL 指导主要用于**引导 `Cognitor` 创建、查询、链接 Canvas 节点，并与 Canvas 工作流进行交互**。所有 NPL 操作都应被理解为对 `Cognitor` 的**认知引导**，而非直接的 API 调用。

## 2. 核心概念映射

* **Canvas 节点 (`Node`)**: 在 NPL 中，通过 **`NodeRef`** 类型的 `Datum` 句柄来引用。`NodeRef` 指向 Canvas 中具有唯一 `node_id` 的特定 `<Node>`。
...
* **`Canvas`**: (`Datum`)
    ...
    * **引导模式 (概念性)**:
        * `Canvas.find_node(node_id: Datum) -> NodeRef`: 指导 `Cognitor` 在 Canvas 中查找指定 `node_id` 的节点，并返回其 `NodeRef`。
...
* **`nodes`**: (`Datum`, 映射概念)
    * **作用**: 提供通过 `node_id` 访问特定节点 `NodeRef` 的便捷方式。
    * **引导模式**:
        * `nodes[node_id_datum]` (索引访问): 指导 `Cognitor` 获取 `node_id_datum` (其值是节点 ID 字符串) 对应的 `NodeRef`。相当于 `Canvas.find_node(node_id_datum)` 的快捷方式。
* **`operator`**: (`CognitorRef`)
    * **作用**: 始终指向当前持有 Space 控制权并处理 NPL 指导的 `Cognitor`。
* **`origin`**: (`CognitorRef`)
    * **作用**: 在 NPL 指令文本中，指向最初发出该指令的 `Cognitor`。
* **`current_node`**: (`NodeRef`, **新增**)
...
* **`a_node_ref.get_id() -> Datum`**: 指导获取该节点的 `node_id` (字符串 Datum)。
...
* **`Canvas.CreateNode(type: Datum, origin: CognitorRef = operator, attributes: Datum = {}, initial_children: Datum = {}) -> NodeRef`**:
    ...
    * **行为**: `Cognitor` 计算新节点的 `seq` 和 `node_id`，构建 XML 结构，将其追加到 Canvas (逻辑上)，并通过 CT 记录此创建过程。
