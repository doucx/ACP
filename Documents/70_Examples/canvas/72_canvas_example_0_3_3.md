基于 `ACP Canvas 0.3.3`，对常见的Chat环境进行优化。
####  Fhrsk 交互
...
<Canvas>
    ...
    <!-- Arena 处理 User:0，推断路由并记录 -->
    <ArenaLog>
        <log originator="Gemini" type="LLM Agent" log_level="INFO" seq="0">
            <message>处理 User:0 (EXEC)。检测到 'chat' 关键字，推断用户意图为与 Fhrsk 交互。将请求路由至 Fhrsk。</message>
...
<Canvas>
    ...
    <!-- Arena 处理 User:0，遇到 input() -->
    <ArenaLog>
         <log originator="Gemini" type="LLM Agent" log_level="INFO" seq="0">
            <message>开始处理 User:0 (EXEC)。创建新`OUTPUT Cell`。</message>
            <log_entry_type value="StateTransition"/>
         </log>
    </ArenaLog>
    ...
    <!-- Arena 收到用户输入 User:1 -->
     <ArenaLog>
         <log originator="Gemini" type="LLM Agent" log_level="INFO" seq="1">
            <message>收到来自 User:1 的 INPUT。内容为 "Alice"。现在可以继续执行 User:0 中 `input()` 之后的代码。</message>
            <log_entry_type value="StateTransition"/>
         </log>
    </ArenaLog>
    ...
    <Cell originator="Gemini" type="OUTPUT" seq="1">
        ...
        <value>成功</value> <!-- 表示 User:0 的整个 Cell 执行成功 -->
    </Cell>
    ...
    <Cell originator="User" type="EXEC" seq="2">
     ...
     <ArenaLog>
         <log originator="Gemini" type="LLM Agent" log_level="INFO" seq="2">
            <message>开始处理 User:2 (EXEC)。</message>
            <log_entry_type value="ActionPlan"/>
        </log>
         <log originator="Gemini" type="LLM Agent" log_level="INFO" seq="3">
             <message>User:2 未提供 <depends_on>。根据交互上下文，推断此 Cell 是一个新的执行起点，不依赖于之前的特定 Cell (但可能访问之前的状态，如变量 'name')。</message>
    ...
    <Cell originator="Gemini" type="OUTPUT" seq="2">
	    <depends_on>
         ...
         <log originator="Gemini" type="LLM Agent" log_level="DEBUG" seq="0">
            <message>执行 User:2 中的代码: `name`。</message>
            <log_entry_type value="ActionPlan"/>
        </log>
        <log originator="Gemini" type="LLM Agent" log_level="DEBUG" seq="1">
