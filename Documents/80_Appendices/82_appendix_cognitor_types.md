...
*   **协议要求的补偿机制**:
    *   必须通过`Logs`系统完整记录当前`Arena`内的认知过程
...
*   **核心特性**:
    ...
    *   **具身能力 (Embodied Capability)**:  
        ...
        - **即时反馈调整**: 能在现实交互中实时修正行为（如调试代码、调整实验参数），而非仅依赖预定义的 `Logs` 回溯。  
...
*   **在 ACP 中的角色**:
    ...
    *   可以**修改上下文**，例如，人类可以直接修改已有的 `Canvas` 中的某个 `log` 节点，来控制 `LLM` 模拟的 `Arena` 的行为。
...
*   **扮演者唯一性**: 通常在同一个时刻，一个 `PersonaCognitor` 只能由一个 `Cognitor` 扮演。如果需要切换 `Cognitor` ，需要在日志中显式声明。
